"""Conversation agent for Odysseus Agent."""
...
from .api import OdysseusApiClient, OdysseusApiError, OdysseusStreamSetupError
from .compat import entry_value, resolve_continued_conversation_mode
from .const import (
    CONF_ALWAYS_SPEAK_FALLBACK,
    CONF_ALLOW_BASH,
    CONF_ALLOW_WEB_SEARCH,
...
async def async_setup_entry(
    ...
    """Set up the Odysseus conversation entity."""
    entry_data = hass.data[DOMAIN][entry.entry_id]
    async_add_entities(
        [
            OdysseusConversationAgent(
                hass,
                entry,
                entry_data["client"],
...
class OdysseusConversationAgent(ConversationEntity, AbstractConversationAgent):
    """Odysseus Agent conversation entity for Home Assistant."""
    ...
    def __init__(
        ...
        self._attr_name = getattr(entry, "title", None) or "Odysseus"
    ...
    async def _async_process_inner(
        ...
        # Build system prompt (optional — Odysseus Agent has its own)
        ...
        except OdysseusApiError as err:
            _LOGGER.error("Odysseus API error: %s", err)
            intent_response = intent.IntentResponse(language=user_input.language)
            intent_response.async_set_error(
                intent.IntentResponseErrorCode.UNKNOWN,
                f"Error communicating with Odysseus Agent: {err}",
            )
            return self._build_conversation_result(
                intent_response,
                conv_id,
    ...
    async def _stream_chat_log_response(
        ...
        async def _stream() -> AsyncIterator[dict[str, str]]:
            ...
            try:
                ...
                if chunks:
                    _LOGGER.warning(
                        "Odysseus stream failed after content started; keeping partial response: %s",
                        err,
                    )
                    return
                raise
    ...
    async def _iter_voice_safe_response(
        ...
        """Yield speech-safe assistant text chunks from Odysseus streaming."""
        ...
        try:
            async for chunk in self.client.async_stream_message(
                ...
                allow_web_search=self._allow_web_search(),
                allow_bash=self._allow_bash(),
            ):
                stream_started = True
                if safe_chunk := speech_filter.feed(chunk):
                    yield safe_chunk
        except OdysseusStreamSetupError as err:
            if stream_started:
                raise
            _LOGGER.debug(
                "Odysseus streaming setup failed; falling back to non-streaming: %s",
                err,
            )
    ...
    async def _get_response(
        ...
        try:
            ...
            if chunks:
                _LOGGER.warning(
                    "Odysseus stream failed after content started; not retrying to avoid duplicate tool calls"
                )
            raise
        return "".join(chunks)
    ...
    def _response_invites_follow_up(self, response_text: str) -> bool:
        """Detect when Odysseus ended with a question worth keeping HA open for."""
    ...
    def _allow_web_search(self) -> bool:
        return bool(
            entry_value(
                self.entry,
                CONF_ALLOW_WEB_SEARCH,
